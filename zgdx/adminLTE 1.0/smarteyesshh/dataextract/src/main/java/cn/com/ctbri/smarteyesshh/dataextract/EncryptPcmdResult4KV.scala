...
object EncryptPcmdResult4KV {
  ...
  def main(args: Array[String]): Unit = {
    val pcmdResultPath = args(0)
    val outputPath = args(1)
    val timestamp = pcmdResultPath.split('.').last
    if (!timestamp.matches("\\d{14}")) {
      println(s"wrong file name! $pcmdResultPath")
      sys.exit(1)
    }
    val date = timestamp.substring(0, 8)

    val locs = Source.fromFile(pcmdResultPath).getLines()

    val writer = new FileWriter(outputPath)
    val lines = locs.toList
    val count = lines.size
    writer.write(s"smarteyes_${date}_loc_count\t$count\n")
    lines.zipWithIndex.foreach {
      case (line, index) =>
        val k = s"smarteyes_${date}_loc_$index"
