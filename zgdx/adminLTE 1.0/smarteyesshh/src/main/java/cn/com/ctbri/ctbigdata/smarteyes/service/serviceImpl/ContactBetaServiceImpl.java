...
/*            phone_list.forEach(s -> {
    ...
    @Override
    public boolean phoneIfExist(String targetPhone) {
        MappingExample mappingExample = new MappingExample();
        int count = 0;
        try {
            mappingExample.createCriteria().andMdnEncryptEqualTo(AESUtil.aesEncrypt(AESUtil.DENGTA_KEY,targetPhone));
            count = mappingMapper.countByExample(mappingExample);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return count > 0;
